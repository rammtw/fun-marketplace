...
- **Пополнение** `POST /api/wallet/deposit` — заглушка вместо платёжного провайдера,
  ...
  `parseAmountToMinor`. Ручка возвращает баланс после зачисления — второй запрос не нужен.
...
src/
  ...
  pages/      games, game, offer, order, orders, my-offers, offer-editor, wallet,
...
- `wallet` — баланс нужен шапке, панели покупки и кошельку, и после каждой покупки
  ...
  деньги; теперь один `WalletProvider`: `refresh()` после покупки, `apply()` кладёт
  баланс из ответа пополнения. Транспорт при этом остался в `shared/api`.
...
зовут несколько слайсов: каталог (`catalog.ts` — витрина и редактор лота), кошелёк и
рабочий стол продавца (`seller-offers.ts`). Импорты абсолютные от `src` (`baseUrl` в `tsconfig.json`),
