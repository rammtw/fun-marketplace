...
- **Регистрация идёт под редакцию политики.** Форма сначала берёт действующую
  редакцию (`GET /api/privacy/policy`, открыта всем) и показывает её текст, а
  в `POST /api/auth/register` возвращает `policyVersion` вместе с согласиями:
  `personalDataConsent` и `isAdult` обязательны и по умолчанию сняты,
  `publicProfileConsent` (показ профиля в открытой части) и `marketingConsent` —
  по желанию. Без политики регистрировать некого, поэтому при неудачной загрузке
