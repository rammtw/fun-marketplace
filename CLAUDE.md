...
карточка лота с покупкой, список заказов и карточка заказа, кошелёк, регистрация и вход. Бэкенд лежит рядом — `../symfony-universe` (JSON API + JWT,
...
src/
  ...
  pages/      games, game, offer, order, orders, wallet, login, register, confirm-email, not-found
...
- `order` — словарь статусов заказа: его читают и список, и карточка, а меняется он
  вместе с доменом, а не с экраном.
...
зовут несколько слайсов. Импорты абсолютные от `src` (`baseUrl` в `tsconfig.json`),
